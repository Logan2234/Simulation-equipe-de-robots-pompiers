            ...
            for (Incendie incendie : donnees.getIncendies()) {
                ...
                if (!robotsListe.isEmpty()) {
                    ...
                    for (Robot robot : robotsListe) {
                        // Si le robot est vide, on l'envoie se remplir et il ne s'occupe plus de
                        // l'incendie
                        if (robot.getReservoir() == 0) {
                            ...
                            try {
                                ...
                                System.out
                                        .println("Pas d'eau dans la carte. Le robot ne peux pas remplir le réservoir.");
                            }
                        }
    ...
    protected void gestionIncendies(Incendie incendie) {
        ...
        for (Robot robot : donnees.getRobots()) {
            ...
            if (!occupes.contains(robot)) {
                // On essaie de l'envoyer sur l'incendie et voir le temps nécessaire pour y
                // aller
                try {
                    Chemin chemin = CalculPCC.dijkstra(carte, robot.getPosition(), incendie.getPosition(), robot,
                            robot.getLastDate());
        ...
        // Si on a trouvé un robot disponible, ça veut dire que robotAMobiliser est le
        // plus proche de
    ...
    private Chemin ouAllerRemplirReservoir(Robot robot) throws NoWaterException, NoPathAvailableException {
        ...
        for (Case caseEau : casesAvecEau) {
            if (robot.getCapacite() == 10000) { // Si c'est un drone : remplissage au-dessus de l'eau
                try {
                    Chemin cheminVersEau = CalculPCC.dijkstra(carte, positionRobot, caseEau, robot,
                            robot.getLastDate());
            ...
            } else { // Pour les autres robots qui se remplissent à côté de l'eau (et non pas
                     // au-dessus)
                ...
                for (Direction direction : Direction.values()) {
                    try {
                        ...
                        if (positionRobot.getCarte().voisinExiste(caseEau, direction, robot)) {
                            ...
                            // Si on ne peut pas accéder à la case par la direction voulue on cherche une
                            // autre direction
                            continue;
                        }
                    } catch (NoPathAvailableException e) {
                        ...
                        // Si on ne peut pas accéder à la case on cherche un autre robot (cette
                        // exception
